...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        particleDistribution = self.numParticles/len(self.legalPositions)
        self.particles = self.legalPositions * particleDistribution
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in beliefDist:
                ...
                beliefDist[particle] *= emissionModel[trueDistance]

            if not beliefDist.totalCount():
                self.initializeUniformly(gameState)
            else:
                beliefDist.normalize()
                self.particles = [util.sample(beliefDist) for _ in xrange(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            newPostDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        particleDistribution = self.numParticles;
        self.particles = []
        ...
        for i in range(len(cartesian_product)):
        	if (particleDistribution >= len(cartesian_product)):
        		self.particles = self.particles + cartesian_product
        		particleDistribution = particleDistribution - len(cartesian_product)
        	else:
        		break
